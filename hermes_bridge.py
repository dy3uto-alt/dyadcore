...
def format_for_prompt(results: list, dyadcore=None, title: str = "关系场背景") -> str:
    ...
    自动分离 beliefs（当前理解）和 memories（原始痕迹）：
      - Beliefs 渲染为 "当前理解" 节：Agent 对用户的认知快照
      - Memories 渲染为 "关系场背景" 节：原始对话痕迹 + 演化链
    ...
    # --- 分离 beliefs 和 memories ---
    belief_results = [r for r in results if r.get('is_belief')]
    memory_results = [r for r in results if not r.get('is_belief')]

    output_parts = []

    # --- 渲染 Beliefs：当前理解 ---
    if belief_results:
        belief_lines = ["## 当前理解（Agent 对用户的认知，可被修正）", ""]
        for i, b in enumerate(belief_results, 1):
            statement = b.get('statement', b.get('content', ''))
            ev_count = len(json.loads(b.get('evidence_ids', '[]')))
            conf_pct = int(b.get('confidence', 0.5) * 100)
            status = b.get('verification_status', 'unverified')
            field_tag = b.get('field', '') or ''
            # 已确认的信念加标记
            status_mark = " [已确认]" if status == 'confirmed' else ""
            belief_lines.append(
                f"{i}. [{field_tag} | {conf_pct}%置信 | {ev_count}条证据{status_mark}] {statement}"
            )
        output_parts.append("\n".join(belief_lines))
    ...
    if memory_results:
        ...
        if dyadcore is not None and len(memory_results) >= 2:
            result_ids = [r['id'] for r in memory_results]
        ...
        standalone = [r for r in memory_results
                      if r['id'] not in chained_old and r['id'] not in chained_new]
        chains = list(contradicted_map.items())
        ...
        if chains:
            ...
            for old_id, new_id in chains:
                old_mem = next((r for r in memory_results if r['id'] == old_id), None)
                new_mem = next((r for r in memory_results if r['id'] == new_id), None)
